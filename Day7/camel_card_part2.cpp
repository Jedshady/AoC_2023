...
string fromCardCount2HandType(map<char, int> &card2count) {
    string current_type;
...
string determineTypeWithJ(const string &current_hand) {
    string current_type;
    ...
    card2count[maxElement->first] += J_occurrence;

    return fromCardCount2HandType(card2count);
}

string determineTypeWithoutJ(const string &current_hand) {
...
void classifyHand2Type(map<string, vector<pair<string, int>>> &type2hand, const string &line) {
    ...
    string hand_type; 
    if (current_hand.find('J') == string::npos) {
        hand_type = determineTypeWithoutJ(current_hand);
    } else {
        hand_type = determineTypeWithJ(current_hand);
    }
...
map<char, int> createCardRankMap() {
    ...
    const string ranks = "AKQT98765432J";
...
bool compareCards(const pair<std::string, int>& a, const pair<std::string, int>& b, const map<char, int>& rankMap) {
    ...
    // Longer string (more cards) is considered higher if ranks are equal
    // Though in this case, strings are of equal length
...
int main(int argc, char* argv[]) {
    ...
    cout << "Part2 total winning: " << total_winning << endl;
